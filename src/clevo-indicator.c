...
int get_fan_duty(int temp, int duty)
{
    if (tempQueue.size == 0) {
        tempQueue.data = (int *) malloc(QUEUE_SIZE * sizeof(int));
        memset(tempQueue.data, 0, QUEUE_SIZE * sizeof(int));
    }

    if (tempQueue.size < QUEUE_SIZE) {
        tempQueue.data[tempQueue.size++] = temp;
    } else {
        for (int i = 1; i < QUEUE_SIZE; ++i) {
            tempQueue.data[i - 1] = tempQueue.data[i];
        }
        tempQueue.data[QUEUE_SIZE - 1] = temp;
    }

    int t = 0;
    int s = QUEUE_SIZE;
    for (int i = 0; i < QUEUE_SIZE; ++i) {
        if (tempQueue.data[i] != 0)
            t += tempQueue.data[i];
        else --s;
    }

    int auto_duty = t / s + 17;
    if (s != 0)
        return auto_duty > 100 ? 100 : auto_duty;
    else return 0;
}
